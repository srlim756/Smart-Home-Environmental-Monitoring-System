...
int main(int argc, char *argv[]) {
    ...
    if (fd < 0) {
        fprintf(stderr, "[HUMI] Failed to connect to hub\n");
        return 1;
    }
    printf("[HUMI] Connected via %s\n", use_tcp ? "TCP" : "UDS");

    int counter = 0;
    while (1) {
        sensor_data_t data;
        data.type = SENSOR_HUMI;
        data.value = sensor_random_walk(HUMI_BASE, 10.0f, &counter, 0.0f);
        data.timestamp = (uint32_t)(time(NULL));

        if (ipc_send(fd, &data, sizeof(data)) < 0) {
            fprintf(stderr, "[HUMI] Connection lost, reconnecting...\n");
            close(fd);
            sleep(1);
