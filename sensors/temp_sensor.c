...
int main(int argc, char *argv[]) {
    ...
    if (fd < 0) {
        fprintf(stderr, "[TEMP] Failed to connect to hub\n");
        return 1;
    }
    printf("[TEMP] Connected via %s\n", use_tcp ? "TCP" : "UDS");

    int counter = 0;
    while (1) {
        sensor_data_t data;
        data.type = SENSOR_TEMP;
        data.value = sensor_random_walk(TEMP_BASE, 3.0f, &counter, 0.6f);
        data.timestamp = (uint32_t)(time(NULL));

        if (ipc_send(fd, &data, sizeof(data)) < 0) {
            fprintf(stderr, "[TEMP] Connection lost, reconnecting...\n");
            close(fd);
            sleep(1);
